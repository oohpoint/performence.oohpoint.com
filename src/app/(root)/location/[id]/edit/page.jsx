...
import { Building2, MapPin, Users, Shield, Save, Loader, CheckCircle2, ArrowLeft, AlertCircle } from "lucide-react";
...
export default function LocationEditPage() {
    ...
    useEffect(() => {
        ...
        const fetchLocationData = async () => {
            try {
                ...
                reset(
                    {
                        ...
                        college_type: data.college?.type || undefined,
                        college_tags: Array.isArray(data.college?.tags) ? data.college?.tags : [],
                        sub_location_type: data.sub_location_type || undefined,
                        cafe_sub_categories: Array.isArray(data.cafe_sub_categories) ? data.cafe_sub_categories : [],
                        city: (data.city || "").trim(),
                        area: data.area || "",
                        ...
                        restricted_categories: Array.isArray(data.restricted_categories) ? data.restricted_categories : [],
                    },
                    { shouldDirty: false }
                );
                ...
                setLoadError(error.message || "Failed to load location");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    useEffect(() => {
        if (isInitialized.current && locationType === "Food & Beverage") {
            setValue("cafe_sub_categories", []);
        }
    }, [subLocationType, setValue, locationType]);
        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 font-semibold">Loading location details...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="max-w-md mx-auto px-4">
                    ...
                    <p className="text-gray-600 text-center mb-6">{loadError}</p>
                    <button
    ...
    return (
        <div className="bg-white">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                <form onSubmit={handleSubmit(onSubmit)} className="py-8 space-y-10">
                    {/* Location Identity */}
                    <FormSection title="Location Identity" icon={Building2}>
                            ...
                            {locationType === "College" && (
                                <>
                                    ...
                                    <CheckboxGroupField
                                        ...
                                        onChange={(value) => {
                                            const updated = collegeTags.includes(value)
                                                ? collegeTags.filter((v) => v !== value)
                                                : [...collegeTags, value];
                                            setValue("college_tags", updated);
                                        }}
                                    />
                                </>
                            )}
                                ...
                                <>
                                    ...
                                    {subLocationType && (
                                        <CheckboxGroupField
                                            ...
                                            onChange={(value) => {
                                                const updated = cafeSubCategories.includes(value)
                                                    ? cafeSubCategories.filter((v) => v !== value)
                                                    : [...cafeSubCategories, value];
                                                setValue("cafe_sub_categories", updated);
                                            }}
                                        />
                                    )}
                                </>
                    ...
                    {/* Geo Coordinates */}
                    <FormSection title="Geo Coordinates & Radius" icon={MapPin}>
                    ...
                    {/* Audience Profile */}
                    <FormSection title="Audience Profile" icon={Users}>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            <SelectField
                    ...
                    {/* Context & Safety */}
                    <FormSection title="Context & Safety" icon={Shield}>
                    ...
                    <div className="flex gap-4 pt-8 border-t border-gray-200">
                        <button
                            ...
                            className="flex items-center gap-2 px-6 py-3 rounded-xl border border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <ArrowLeft size={20} />
                            Back
                        </button>
                        <button
                            type="submit"
                            disabled={isSubmitting}
                            className="flex-1 px-6 py-3 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {isSubmitting ? (
                                <>
